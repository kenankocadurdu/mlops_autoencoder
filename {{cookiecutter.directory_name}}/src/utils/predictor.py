...
class Executor:
    def __init__(self, path_testset: str, save_path: str, path_model: str, normalize: bool, path_trainset: str, production: bool) -> None:
    ...
    def _run_inference(self, model_generator, data_loader: DataLoader, batch_size: int) -> None:
        ...
        with torch.no_grad():
            for i, (images, _) in enumerate(tqdm(data_loader, 0)):
    ...
    def _save_output_images(self, outputs, images, data_loader, batch_idx, batch_size, iteration):
        for k in range(batch_size):
            ...
            fname, _ = data_loader.dataset.images[batch_idx * batch_size + k]
